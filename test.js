const defered = require('./index');

const t1 = defered.sync((defer) => (name) => {
    defer((result, e) => {
        if (result !== 'hello,' + name) throw new Error('t1 failed');
        if (e) throw new Error('t1 failed', e);
        return 'nihao,' + name;
    });
    return 'hello,' + name;
});

const t2 = defered.sync((defer) => (name) => {
    defer((result, e) => {
        if (result !== undefined) throw new Error('t2 failed');
        if (e.message !== 'test error') throw new Error('t2 failed');
        return 'nihao,' + name;
    });
    throw new Error('test error')
});

const t3 = defered.async((defer) => async (name) => {
    defer(async (result, e) => {
        if (result !== 'hello,' + name) throw new Error('t3 failed');
        if (e) throw new Error('t3 failed', e);
...
const t4 = defered.async((defer) => async (name) => {
    defer(async (result, e) => {
